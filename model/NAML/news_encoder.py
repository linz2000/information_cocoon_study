...
class ElementEncoder(torch.nn.Module):
    ...
    def forward(self, element):
        try:
            tmp = self.embedding(element)
        except Exception as e:
            print(e)
            print(element)
        return F.relu(self.linear(tmp))
...
class NewsEncoder(torch.nn.Module):
    def __init__(self, config, pretrained_word_embedding):
        ...
        category_embedding = nn.Embedding(config.num_categories,
                                          ...
                                          padding_idx=0)
        # add
        subcategory_embedding = nn.Embedding(config.num_subcategories,
                                          config.category_embedding_dim,
                                          padding_idx=0)
        ...
        for name in (set(config.dataset_attributes['news']) & set(element_encoders_candidates)):
            if name == 'category':
                model_dict[name] = ElementEncoder(category_embedding, config.category_embedding_dim,config.num_filters)
            elif name == 'subcategory':
                model_dict[name] = ElementEncoder(subcategory_embedding, config.category_embedding_dim, config.num_filters)
        self.element_encoders = nn.ModuleDict(model_dict)
